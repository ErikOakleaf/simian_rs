    ...
    fn compile_statement(&mut self, statement: &Statement) -> Result<(), CompilationError> {
        match statement {
            ...
            Statement::Let(let_statement) => {
                let name = &let_statement.name.literal_string(self.input);
                ...
                let symbol = self
                    .symbol_table
                    .borrow_mut()
                    .define(&let_statement.name.literal_string(self.input));
            ...
            Statement::Assign(assign_statement) => match assign_statement.target.as_ref() {
                Expression::Identifier(identifier_expression) => {
                    ...
                    let symbol = self
                        .symbol_table
                        .borrow_mut()
                        .resolve(&identifier_expression.literal_string(self.input))?;
    ...
    fn compile_expression(&mut self, expression: &Expression) -> Result<(), CompilationError> {
        match expression {
            ...
            Expression::FloatLiteral(float_value) => {
                let float = Object::Float(float_value.value);
            ...
            Expression::Identifier(token) => {
                let symbol = self
                    .symbol_table
                    .borrow_mut()
                    .resolve(&token.literal_string(self.input))?;
            ...
            Expression::Function(function_literal_expression) => {
                ...
                if let Some(ref name) = function_literal_expression.name {
                    self.symbol_table
                        .borrow_mut()
                        .define_function_name(&name.literal_string(self.input));
                }

                for parameter in function_literal_expression.parameters.iter() {
                    self.symbol_table
                        .borrow_mut()
                        .define(&parameter.literal_string(self.input));
                }
            ...
            Expression::Infix(infix_expression) => {
                let operator = infix_expression.token.literal_string(self.input);
            ...
            Expression::Prefix(prefix_expression) => {
                let operator = prefix_expression.token.literal_string(self.input);
